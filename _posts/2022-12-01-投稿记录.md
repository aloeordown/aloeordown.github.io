...
QRS有四个审稿意见，主要还是实验评估不行，motivation也不太清楚。
...
Detailed Comments
---------------------------------------------------------------------------
Authors of the papers present an approach to improve the Easy Approach to Requirements
Systax (EARS) named EARS+, highlight the new characteristics like time and data in its
description, and the possible improvement of the security characteristic of systems like
Cyber-Physical Systems (CPS), using theirs approach. They have also designed a test
framework based on a test model with some algorithms to convert the EARS+ requirements
into a describing test framework based. To finish, they have evaluated their approach using
three use cases with good results.
In my opinion, this paper addresses a current issue. The authors address clearly the problem
and the approach to solve the problem found in the Easy Approach to Requirements Syntax.
I would like that the authors consider the following suggestions to improve the paper.
 Checking the writing. In the text, there are some duplicate words, in the abstract and
related works.
 In the text there are several acronyms that are not defined as "OCL" and are
necessary for a good understanding of the text
---------------------------------------------------------------------------
```
